...
ui <- fluidPage(
  ...
  sidebarLayout(
    ...
    sidebarPanel(
      ...
      selectInput(inputId = "subset_income", label = "Choose an income"),
      # Interactive piece 2: inputId = "set_yaxis"
      selectInput(inputId = "set_yaxis", label = "Y axis")),
...
server <- function(input, output) {
  ...
  create_subset <- reactive(income %>%
                              # Only include capital loss is greater than 0
  ...
  output$myfigure <- renderPlot(ggplot(create_subset()) +
                                  ...
                                  boxplot(aes_string(x = "occupation", y = input$set_yaxis)) +
                                  theme_bw(18) +
